...
const TABS: { tab: MobileTab; icon: ReactNode }[] = [
  ...
  { tab: 'gear', icon: <path d="M7 7.5 h8 v10.5 a2 2 0 0 1 -2 2 h-4 a2 2 0 0 1 -2 -2 z M9.5 7.5 v-3 h3 v3 M7 12 h8" /> },
  { tab: 'mix', icon: <path d="M6.5 5 h9 l-1.1 12.2 a2 2 0 0 1 -2 1.8 h-2.8 a2 2 0 0 1 -2 -1.8 z M7.2 11.5 h7.6" /> },
  {
    tab: 'food',
    icon: <path d="M4.4 6.2 C3.9 14.2 9.6 19.3 18 18.2 C18.9 18.1 19.2 17.1 18.4 16.6 C12.3 14.5 7.6 11.6 7.2 6.4 C7.1 5.6 4.5 5.4 4.4 6.2 Z M5.6 5.9 L5.1 3.6" />,
  },
];
...
function labelStyle(active: boolean): CSSProperties {
  return { fontSize: 9, fontWeight: 600, whiteSpace: 'nowrap', letterSpacing: '-0.01em', color: active ? 'var(--ink)' : '#9AA09B' };
}
...
export function MobileApp() {
  ...
  return (
    <div className="mobile-app-height" style={{ display: 'flex', flexDirection: 'column', position: 'relative', background: 'var(--surface)' }}>
      ...
      <div style={{ flexShrink: 0, padding: '8px 8px 16px', display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 2 }}>
        {TABS.map(({ tab: t2, icon }) => {
          const active = tab === t2;
          const label = t2 === 'plan' ? strings.tabPlan : t2 === 'gear' ? strings.tabGear : t2 === 'mix' ? strings.tabMix : strings.tabFood;
          return (
            <button
              key={t2}
              type="button"
              onClick={() => setTab(t2)}
              style={{ border: 'none', background: 'transparent', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, padding: '4px 0', cursor: 'pointer' }}
            >
              <svg width={21} height={21} viewBox="0 0 22 22" fill="none" stroke="currentColor" strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round" style={iconStyle(active)}>
                {icon}
              </svg>

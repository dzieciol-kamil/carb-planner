...
const cardStyle: CSSProperties = { border: '1px solid #EDEFEA', borderRadius: 11, overflow: 'hidden' };
...
const fillBlockStyle: CSSProperties = { padding: '8px 0', borderBottom: '1px solid #F2F3EF' };

export function MobileRecipesSection() {
  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 16, padding: 14 }}>
      <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 10 }}>{strings.recipes}</div>
  ...
  return (
    <div style={cardStyle}>
      <div style={cardHeadStyle}>
        ...
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, opacity: 0.85 }}>
  ...
  return (
    <div style={fillBlockStyle}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        {lines.map((line) => (
          <div key={line.k} style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8 }}>
            <span style={{ fontSize: 11, color: 'var(--muted-2)' }}>{line.k}</span>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 600 }}>{line.v}</span>
          </div>
        ))}
      </div>
    </div>

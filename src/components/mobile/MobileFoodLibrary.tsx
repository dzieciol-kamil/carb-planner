...
const fieldWrapStyle: CSSProperties = { display: 'flex', flexDirection: 'column', gap: 4, flex: '1 1 auto' };
const fieldLabelStyle: CSSProperties = { fontSize: 9, fontWeight: 700, textTransform: 'uppercase', color: 'var(--muted-3)', letterSpacing: '0.04em' };
const fieldBoxStyle: CSSProperties = { display: 'flex', alignItems: 'center', gap: 4, border: '1px solid var(--chip-border)', borderRadius: 10, padding: '0 10px', background: '#fff', height: 44 };
const numberInputStyle: CSSProperties = { width: '100%', border: 'none', background: 'transparent', fontFamily: "'JetBrains Mono', monospace", fontSize: 13, fontWeight: 600 };
...
export function MobileFoodLibrary() {
  ...
  return (
    <div style={{ padding: '12px 14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{strings.foodSection}</div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6 }}>
              <label style={fieldWrapStyle}>
                ...
                <span style={fieldBoxStyle}>
                  <NumberInput min={0} step={1} value={entry.carbs} onChange={(carbs) => updateFoodLibEntry(entry.key, { carbs: Math.max(0, carbs) })} style={numberInputStyle} />
              ...
              <label style={fieldWrapStyle}>
                ...
                <span style={fieldBoxStyle}>
                  <NumberInput min={0} step={10} value={entry.ml || 0} onChange={(ml) => updateFoodLibEntry(entry.key, { ml: Math.max(0, ml) })} style={numberInputStyle} />
                  <span style={{ fontSize: 10, color: 'var(--muted-3)' }}>ml</span>
                </span>
              </label>
              <button
                type="button"
                onClick={() => updateFoodLibEntry(entry.key, { cont: !entry.cont, span: entry.span || 18 })}
                style={{
                  flex: '0 0 auto',
                  height: 44,
                  padding: '0 10px',
              ...
              <button
                ...
                style={{ flex: '0 0 34px', height: 44, border: 'none', background: 'transparent', color: '#B0B5B0', cursor: 'pointer', fontSize: 14, marginLeft: 'auto' }}
              >
                ✕
              </button>
            </div>

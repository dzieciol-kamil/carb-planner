...
type NumericSampleKey = 'intake' | 'absorbed' | 'gut' | 'ml' | 'need' | 'rate' | 'needRate' | 'fluidRate' | 'sweatRate';

function polyline(samplesArr: Sample[], key: NumericSampleKey, px: (x: number) => number, py: (y: number) => number): string {
  return samplesArr.map((p, i) => (i ? 'L' : 'M') + px(p.x).toFixed(1) + ' ' + py(p[key]).toFixed(1)).join(' ');
}
...
export function Chart({ height, showAxis }: ChartProps) {
  ...
  const area = polyline(S, yk, px, py) + ' L' + WIDTH + ' ' + (height - PB) + ' L0 ' + (height - PB) + ' Z';
  ...
  return (
    <svg viewBox={`0 0 ${WIDTH} ${height}`} preserveAspectRatio="none" style={{ width: '100%', height: `${height}px`, display: 'block', overflow: 'visible' }}>
      ...
      <ElevationLayer pts={prof(route).pts} distanceKm={D} width={WIDTH} height={height} bottomPadding={PB} share={showAxis ? 0.62 : 0.7} visible={route.useGpx} />
      ...
      {fills.map((f) => {
        ...
        const color = f.content === 'water' ? CHART_COLORS.water : f.content === 'gel' ? CHART_COLORS.gel : CHART_COLORS.carb;
        return (
          <g key={key} opacity={on ? 1 : 0.34}>
            <rect x={px(f.from)} y={0} width={Math.max(1, px(f.to) - px(f.from))} height={height - PB} fill={color} opacity={on ? 0.14 : 0.03} />
            <line
              x1={px(f.from)}
              x2={px(f.from)}
              y1={0}
      ...
      {ticks.map((k, i) => (
        <line key={'g' + i} x1={px(k)} x2={px(k)} y1={4} y2={height - PB} stroke="#EDEFEA" strokeWidth={1} vectorEffect="non-scaling-stroke" />
      ))}
      <line x1={0} x2={WIDTH} y1={height - PB} y2={height - PB} stroke="#DDE0DA" strokeWidth={1} vectorEffect="non-scaling-stroke" />
      ...
      {showAxis &&
        ticks.map((k, i) => (
          <Fragment key={'t' + i}>
            <text x={px(k) + 4} y={height - 6} fill={CHART_COLORS.muted} fontSize={11} fontFamily="JetBrains Mono, monospace">
